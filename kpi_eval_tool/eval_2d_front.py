...
class Eval2DFront:
    def __init__(self, lable_path, perce_path, ori_pic_path):
        ...
        self.recall_error_path = self.current_path + '/' + self.str_time + '/front_2d_recall_error'
        self.precision_error_path = self.current_path + '/' + self.str_time+ '/front_2d_precision_error'
    ...
    def eval_2d_front_recall(self): 
        ...
        for i in range(len(self.lable_jsons_list)):
            ...
            for perce_result in perce_json_data_boxs:
                ...
                perce_all_box_list.append(perce_box)
            ...
            for lable_tag in utils.get_lable_2d_boxs(lable_json_data):
                ...
                if lable_type not in self.obstacle_type:
                    continue 
                lable_box,lx,ly,w,h = utils.front_2d_get_lable_box(lable_tag)
            ...
            if recall_error_list!=[]:
                ...
                print_json_comment["perce_box"] = perce_all_box_list
    ...
    def eval_2d_front_precision(self):
        '''评测前视2d的precision指标'''
        df = pd.DataFrame(columns=['KPI'] + self.obstacle_type)
        df.loc[0, 'KPI'] = '检测类别正确数量'
        df.loc[1, 'KPI'] = '检测类别错误数量'
        df.loc[2, 'KPI'] = '检测出的总数量'
        df.loc[3, 'KPI'] = '精确率'
        df.fillna(0, inplace=True)
        ...
        for i in range(len(self.lable_jsons_list)):
            ...
            for lable_tag in utils.get_lable_2d_boxs(lable_json_data):
                ...
                iou_max_id = iou_max_item[0]
                if iou_max_value >= self.iou:
                    perce_type_value = perce_box_type_list[iou_max_id]
                    if int(perce_type_value)>7:
                        continue
        ...
        for type in self.obstacle_type:
            df.iloc[2,df.columns.get_loc(type)] = df.iloc[0,df.columns.get_loc(type)] + df.iloc[1,df.columns.get_loc(type)]
            df.iloc[3,df.columns.get_loc(type)] = 0 if df.iloc[2,df.columns.get_loc(type)]==0 else round((df.iloc[0,df.columns.get_loc(type)])*100/df.iloc[2,df.columns.get_loc(type)],3)
    ...
    def draw_recall_pic(self):
        ...
        绿色是标注结果，蓝色是感知结果
        '''
        Eval2DFront.get_oripic_depend_errorjson(self, self.recall_error_path,self.ori_pic_path)
        ...
        for root,_,files in os.walk(os.path.join(self.recall_error_path,'json')):
            for file_ in files:
                ...
                for perce_box in perce_boxs:
                    x,y,x1,y1 = utils.front_2d_get_box_point(perce_box)                
                    cv2.rectangle(img_data,(x,y),(x1,y1),(255,0,0),1)
                cv2.imwrite(img_dstpath, img_data)

    def draw_precision_pic(self):
            Eval2DFront.get_oripic_depend_errorjson(self, self.precision_error_path,self.ori_pic_path)
            precision_type_err_pic = os.path.join(self.precision_error_path,'type_err_pic')
            os.makedirs(precision_type_err_pic)
